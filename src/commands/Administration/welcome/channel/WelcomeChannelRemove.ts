...
export default class WelcomeChannelRemove extends Subcommand {
    ...
    async execute(interaction: CommandInteraction, client: BotClient): Promise<void> {
        ...
        if (!guild.config.welcome?.channel) {
            await interaction.reply({ content: "The channel to send join messages in has not been set yet.", ephemeral: true });
            return;
        }

        await client.database.guilds.updateOne({ id: guild.id }, { "$unset": { "config.welcome.channel": "" } });
        await interaction.reply("The channel to send join messages in has been removed.");
    }
}
