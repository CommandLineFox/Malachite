...
export default class DuplicateLogSet extends Subcommand {
    ...
    async execute(interaction: CommandInteraction, client: BotClient): Promise<void> {
        ...
        if (guild.config.duplicates?.log === channel.id) {
            await interaction.reply({ content: "The channel for logging duplicates has already been set to the same channel.", ephemeral: true });
            return;
        }

        if (channel.type !== ChannelType.GuildText) {
            await interaction.reply({ content: "The channel for logging duplicates can only be a text channel.", ephemeral: true })
            return;
        }

        await client.database.guilds.updateOne({ id: guild.id }, { "$set": { "config.duplicates.log": channel.id } });
        await interaction.reply(`The channel for logging duplicates has been set to <#${channel.id}>.`);
    }
}
