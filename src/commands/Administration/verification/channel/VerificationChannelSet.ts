...
export default class DuplicateLogSet extends Subcommand {
    ...
    async execute(interaction: CommandInteraction, client: BotClient): Promise<void> {
        ...
        if (guild.config.verification?.channel === channel.id) {
            interaction.reply({ content: "The channel for accepting verifications has already been set to the same channel.", ephemeral: true });
            return;
        }

        if (channel.type !== "GUILD_TEXT") {
            interaction.reply({ content: "The channel for accepting verifications can only be a text channel.", ephemeral: true })
            return;
        }

        await client.database.guilds.updateOne({ id: guild.id }, { "$set": { "config.verification.channel": channel.id } });
        interaction.reply(`The channel for accepting verifications has been set to <#${channel.id}>.`);
    }
}
